...
FROM (
    SELECT
        ...
        RANK() OVER (PARTITION BY year, month ORDER BY total_order_value DESC, customer_id) AS rank
    FROM (
        SELECT
            strftime('%Y', orders.ordered_at) AS year,
            strftime('%m', orders.ordered_at) AS month,
            orders.customer_id,
            SUM(order_line_items.unit_price * order_line_items.quantity) AS total_order_value,
            RANK() OVER (PARTITION BY strftime('%Y-%m', orders.ordered_at) ORDER BY SUM(order_line_items.unit_price * order_line_items.quantity) DESC, orders.customer_id) AS rank
        FROM
            orders
        JOIN
            order_line_items ON orders.order_id = order_line_items.order_id
        GROUP BY
            strftime('%Y', orders.ordered_at),
            strftime('%m', orders.ordered_at),
            orders.customer_id
    )
    WHERE
        rank = 1
