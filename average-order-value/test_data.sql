...
SELECT customers.name, COUNT(value_per_order.order_id) as num_orders, SUM(value_per_order.total_order_value) as total_order_value, CAST(AVG(value_per_order.total_order_value) as INTEGER) as avg_order_value 
FROM customers 
JOIN (SELECT 
      orders.customer_id,
      orders.order_id,
      ...
      GROUP BY orders.order_id) AS value_per_order
ON customers.customer_id=value_per_order.customer_id
